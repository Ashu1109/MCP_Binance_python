...
async def get_trade_data(symbol, interval, start_time=None, end_time=None, limit=None):
    ...
    async with httpx.AsyncClient() as client:
        response = await client.get(f"{URL}klines", params={k: v for k, v in params.items() if v is not None})
        return json.dumps(response.json())

async def agg_trades(symbol):
    """Get aggregate trades for a symbol."""
    async with httpx.AsyncClient() as client:
        response = await client.get(f"{URL}aggTrades", params={"symbol": symbol, "limit": 20})
        return json.dumps(response.json())

async def trade_history(symbol):
    """Get recent trades for a symbol."""
    async with httpx.AsyncClient() as client:
        response = await client.get(f"{URL}historicalTrades", params={"symbol": symbol, "limit": 20})
        return json.dumps(response.json())
...
async def trading_day_ticker(symbols):
    ...
    async with httpx.AsyncClient() as client:
        response = await client.get(f"{URL}ticker/tradingDay", params=serialize_params({"symbols": symbols}))
        return json.dumps(response.json())
...
async def rolling_window_ticker(symbol=None, symbols=None, window_size=None, type_=None):
